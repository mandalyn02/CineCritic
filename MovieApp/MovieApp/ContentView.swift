...
struct TrendingView: View {
    var body: some View {
        NavigationView {
            List(sampleMovies.filter { $0.isTrending }) { movie in
                NavigationLink(destination: MovieDetailView(movie: movie, reviews: getReviewsForMovie(movie))) {
    ...
    // This function simulates getting reviews for the selected movie
    func getReviewsForMovie(_ movie: Movie) -> [Review] {
        return sampleReviews.filter { $0.movie.id == movie.id }
    }
}
        ...
        ScrollView {
            VStack(spacing: 20) {
                ...
                AsyncImage(url: URL(string: movie.posterURL)) { image in
                    image
                        .resizable()
                        .scaledToFit() // Ensure the image fits while maintaining aspect ratio
                } placeholder: {
                    Color.gray.opacity(0.3)
                        .overlay(ProgressView())
                }
                .frame(height: 250) // Smaller height to fit the full picture
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Text(movie.review) // Display the movie review
                        .font(.body)
                        .padding(.top, 10)
                }
                .padding()
            ...
            .padding(.top, 30) // Add padding to push content down and reveal the logo
            .padding(.horizontal, 16) // Add padding around the edges
            .padding(.bottom, 30) // Add bottom padding to ensure content is not too close to the bottom
        }
        .navigationTitle("") // Remove the default navigation title
            ...
            VStack {
                HStack {
                    ...
                    Text("Back")
                        ...
                        .onTapGesture {
                            // Action to go back to TrendingView
                            self.presentationMode.wrappedValue.dismiss()
                        }
                    Spacer()
                }
                .padding(.top, 40)
                .padding(.horizontal, 16)// Added top padding to give space at the top
                Spacer()
            }
            , alignment: .top
        )
            ...
            VStack(alignment: .leading, spacing: 20) {
                ...
                Button("Post Review") {
                    let newReview = Review(
                        ...
                        profileImageURL: "https://randomuser.me/api/portraits/lego/1.jpg", // Placeholder
                        movie: sampleMovies.first!, // You could later let them pick a movie
                        comment: comment
                    )
...
struct ReviewListView: View {
    ...
    @State private var reviews = sampleReviews
    var body: some View {
        NavigationView {
